...
Current bootstrap pins are real upstream identifiers, not placeholders: vLLM `0.30.0`, SGLang `0.5.19`, llama.cpp `v0.5.0`, TensorRT-LLM `1.2.1`, and Ray `2.58.0` for Ray-capable vLLM flavors. Base images are recorded with resolved `sha256:` digests in each manifest.
...
| `llama-cpp-amd-vulkan` | llama.cpp    | AMD    | Vulkan (Mesa RADV/ANV)       | none                                     | Single-node                                            | No ROCm in the image; MXFP4/quantized kernels via Vulkan; also runs on Intel Arc. RDNA4 note: llama.cpp #26663. |
